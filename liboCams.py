...
# This file was downloaded from the withrobot oCam product page.
# See link here: http://withrobot.com/en/camera/ocam-1cgn-u-t/
# I could not find a repository on the comany's github page that hosted this. This
# file was access on Oct 15, 2020. This file will not work with Python 3 on Ubuntu 18
# do to an error in the fcntl package

# Modifications:
#   print functions converted to python3 format
#   Passed Verbose attribute on to v4l2_util class from oCams
#   Modified the FindCamera function so always return index0 for the found device
#   Added 'oCam-1CGN-U-T' SKU to oCams Class mode options
...
class oCams():
  def __init__(self, dev, verbose=0):
    ...
    self.Print(dev,1)
    self.cam = v4l2_util(dev, verbose)
    ...
    if self.cam.card == 'oCamS-1CGN-U':
      ...
      self.mode = 2
    elif self.cam.card == 'oCam-1CGN-U-T':
      self.mode = 2
  ...
  def GetFrame(self, mode=0):
    ...
    if self.mode == 0:
      ...
      return np.ndarray((self.cam.shape[0],self.cam.shape[1], 2), buffer = buff, dtype=np.uint8)
    elif self.mode == 2:
      return np.ndarray((self.cam.shape[0],self.cam.shape[1], 1), buffer = buff, dtype=np.uint8)
    elif self.mode == 3:
      return np.ndarray((self.cam.shape[0],self.cam.shape[1]*2, 1), buffer = buff, dtype=np.uint8)
    else:
      return np.ndarray((self.cam.shape[0],self.cam.shape[1], 2), buffer = buff, dtype=np.uint8)
